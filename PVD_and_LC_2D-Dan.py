...
import sys
sys.path.append('/home/ubuntu/DASH/md_engine/build/python/build/lib.linux-x86_64-2.7')
import time
from DASH import *
import argparse
import numpy as np
...
def pvd_simulation(args):
    ...
    for sim_number in range(args.num_simulations):
        ...
        for replica_number in range(args.num_replicas):
            ...
            writer = WriteConfig(state, handle='writer', fn=args.output + '/' + 'final_PVD_' + args.output + '_' +
            ...
            # Liquid cooling steps
            num_bulk_atoms = 0  # Zero number of bulk atoms
            state.createGroup('bulk_LC')  # Create new bulk group
            state.deactivateFix(fixNVT)  # Remove previous thermostat
            print('Creating Nose Hoover thermostat')
            fixNVT = FixNoseHoover(state, handle='nvt', groupHandle='all')  # Create new thermostat
            fixNVT.setTemperature(temp=1.0, timeConstant=100 * state.dt)
            print('Activating Nose Hoover thermostat')
            state.activateFix(fixNVT)
            integrator.run(10000)  # Heat up the configuration
            state.deactivateFix(fixNVT)
            fixNVT.setTemperature(tempList=[1.0, 0.05], intervals=[0.0, 1.0], timeConstant=100 * state.dt)
            state.activateFix(fixNVT)

            # Record intermediate trajectory positions of film
            writer = WriteConfig(state, handle='writer', fn=args.output + '/' + 'intermediate_LC_' + args.output + '_' +
                                                            str(sim_number) + '_' + str(replica_number), format='xyz',
                                 writeEvery=100)
            state.activateWriteConfig(writer)

            # Record intermediate potential energies of film
            engDataSimple = state.dataManager.recordEnergy(handle='all', mode='scalar', interval=100, fixes=[ljcut])
            tempDataScalar = state.dataManager.recordTemperature(handle='all', mode='scalar', interval=100)

            print('Running cooling')

            integrator.run(args.num_turns_cooling[sim_number])

            print('Cooling done')

            f = open(args.output + '/' + args.PE_file + '_LC_all_' + str(sim_number) + '_' +
                     str(replica_number) + '.txt', 'w')
            f.close()
            for energies, temps in zip(engDataSimple.vals, tempDataScalar.vals):
                f = open(args.output + '/' + args.PE_file + '_LC_all_' + str(sim_number) + '_' +
                         str(replica_number) + '.txt', 'a')
                f.write(str(energies / len(state.atoms)) + '    ' + str(temps) + '\n')
                f.close()
            state.dataManager.stopRecord(engDataSimple)

            state.deactivateFix(fixNVT)
            fixNVT = FixNoseHoover(state, handle='nvt', groupHandle='all')  # Create new thermostat
            fixNVT.setTemperature(temp=0.05, timeConstant=100 * state.dt)
            state.activateFix(fixNVT)
            ...
            # Record final trajectory positions of completed film
            writer = WriteConfig(state, handle='writer', fn=args.output + '/' + 'final_LC_' + args.output + '_' +
                                                            str(sim_number) + '_' + str(replica_number), format='xyz',
                                 writeEvery=1)
            state.activateWriteConfig(writer)

            new_bulk_atoms = [a.id for a in state.atoms if args.bulk_lo <= a.pos[1] <= args.bulk_hi]
            if len(new_bulk_atoms) > 0:
                state.addToGroup('bulk_LC', new_bulk_atoms)
                num_bulk_atoms += len(new_bulk_atoms)
            if num_bulk_atoms > 0:
                engDataSimple = state.dataManager.recordEnergy(handle='bulk_LC', mode='scalar', interval=1,
                                                               fixes=[ljcut])

            integrator.run(1)
            if num_bulk_atoms > 0:
                for energies in engDataSimple.vals:
                    f = open(args.output + '/' + args.PE_file + '_LC_' + str(sim_number) + '_' +
                             str(replica_number) + '.txt', 'a')
                    f.write(str(i) + '    ' + str(energies / float(num_bulk_atoms)) + '\n')
                    f.close()
                state.dataManager.stopRecord(engDataSimple)
            ...
            # Compute inherent structure energy of bulk atoms
            integratorRelax.run(args.num_turns_final_relaxation - 1, 1)
            engDataSimple = state.dataManager.recordEnergy(handle='bulk_LC', mode='scalar', interval=1, fixes=[ljcut])
            integratorRelax.run(1, 1)
            f = open(args.output + '/' + args.EIS_file + '_LC_' + str(sim_number) + '_' +
                     str(replica_number) + '.txt', 'w')
            f.write('# ' + 'Bulk Inherent Structure Energy' + '\n')
            for energies in engDataSimple.vals:
                    f.write(str(energies / float(num_bulk_atoms)))
...
def main():
    ...
    parser.add_argument('--num_turns_cooling', type=int, dest='num_turns_cooling', nargs='*', default=[4700],
                        help='Number of integrator turns for liquid cooling')
    ...
    parser.add_argument('--num_simulations', type=int, dest='num_simulations', default=1,
                        help='Number of simulations with different parameters to run')
    parser.add_argument('--num_replicas', type=int, dest='num_replicas', default=1,
                        help='Number of simulations of a given set of parameters to run')
